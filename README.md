...
- Full-stack dashboard UI (Next.js 16 + React 19 + Tailwind v4 + shadcn/ui) with a LiDAR overview + per-class distribution
- Sample-scoped **Dataset** explorer (browse the sample's B2 objects by pipeline stage) alongside the full-bucket **Files** explorer
...
![Dashboard with LiDAR dataset metrics, per-class distribution, and recent detection runs](docs/images/dashboard.png)

**Ingest** — upload raw KITTI `.bin` / `.pcd` LiDAR frames to B2 as a named sensor log, alongside a full-bucket drag-and-drop uploader.

![Ingest page for uploading raw LiDAR frames to Backblaze B2](docs/images/ingest.png)

**Detection Runs** — create a run against a sensor log and browse the runs library, where each run shows its model, task, status, frame count, and 3D-box total.

![Detection Runs page with a create form and the runs library table](docs/images/runs.png)

**Run detail** — per-frame bird's-eye-view previews and 3D-box annotations with train/val split and write amplification, plus a downloadable dataset manifest and checkpoint record.

![Run detail with per-frame BEV previews and 3D-box annotations](docs/images/run-detail.png)

**Dataset** — a sample-scoped B2 explorer that groups every object by pipeline stage: raw scans, preprocessed tensors, annotations, dataset manifests, and checkpoints.

![Dataset explorer grouping B2 objects by pipeline stage](docs/images/dataset.png)

> **Deploy your own in one click** → [Deploy to Vercel](#deploying-to-vercel). One project, one origin, no CORS to wire up. (The heavy MMDetection3D engine runs locally, not on Vercel — see [docs/features/mmdet3d-engine.md](docs/features/mmdet3d-engine.md).)

## Quick Start

You need: Node.js >= 20, pnpm >= 9, Python >= 3.12, and a free **[Backblaze B2 account](https://www.backblaze.com/sign-up/ai-cloud-storage?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-mmdetection3d-lidar-dataset)**.
...
```bash
git clone https://github.com/backblaze-b2-samples/mmdetection3d-lidar-dataset.git my-cool-app
cd my-cool-app
rm -rf .git
git init
git add .
git commit -m "Initial commit from mmdetection3d-lidar-dataset"
```
...
Open `.env` in your editor and keep it visible. Then head to the [Backblaze B2 dashboard](https://secure.backblaze.com/b2_buckets.htm?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-mmdetection3d-lidar-dataset) and:

1. **Create a bucket.** B2 will show its endpoint like `https://s3.us-west-004.backblazeb2.com` — the middle segment is the region. Paste each into `.env`:
   - **Bucket Unique Name** → `B2_BUCKET_NAME`
   - **Region** (e.g. `us-west-004`, from the endpoint) → `B2_REGION` *(the S3 endpoint is derived from the region — no full endpoint URL to paste)*
2. **Create an application key** with `Read and Write` permission. B2 will show two values — paste each into `.env`:
   - **keyID** → `B2_APPLICATION_KEY_ID`
   - **applicationKey** → `B2_APPLICATION_KEY` *(only shown once — paste it now)*

> Want a walkthrough? See the docs for [creating a bucket](https://www.backblaze.com/docs/cloud-storage-create-and-manage-buckets?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-mmdetection3d-lidar-dataset) and [creating app keys](https://www.backblaze.com/docs/cloud-storage-create-and-manage-app-keys?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-mmdetection3d-lidar-dataset).

**3. Seed a demo sensor log (optional)**

```bash
pnpm run seed --apply          # synthetic KITTI-format frames, offline, zero bundled data
```

This uploads a small demo LiDAR sensor log to `mmdetection3d-lidar-dataset/raw/<sensor_id>/<date>/` so the create-run form has something to select. Set `MMDET3D_USE_DEMO_DATA=1` to instead fetch MMDetection3D's upstream KITTI demo frame at runtime (CC BY-NC, never redistributed) for real detections.
...
That's it. Frontend at `localhost:3000`, API at `localhost:8000`. Ingest frames, create a Detection Run, and run it. Interactive API docs (Swagger UI) are at `localhost:8000/docs`, with ReDoc at `/redoc`.

**5. Install the MMDetection3D engine (opt-in, for real detection)**

```bash
pnpm run setup:mmdet3d-engine  # heavy: torch + OpenMMLab (mmcv builds from source on macOS arm64)
```

The base app runs, ingests frames, previews them, and stays `pnpm verify`-green **without** this step — every engine import is lazy. Running a Detection Run needs the engine; until it's installed, the engine-status badge reads "not installed" and a run fails loudly with an install hint (never a fake result). See [docs/features/mmdet3d-engine.md](docs/features/mmdet3d-engine.md) for the device policy (CPU default, CUDA auto-detect; PointPillars is the CPU-friendly default — SECOND/CenterPoint use sparse-conv that generally needs CUDA/Linux).
...
Use this repository when you are building **3D object-detection / segmentation
datasets from raw LiDAR** and want B2 as the storage layer for raw scans,
derived annotations, checkpoints, and dataset manifests. It is a working sample:
clone it, point it at your own B2 bucket, install the engine, and run
MMDetection3D locally over your sensor logs — with strict architecture, contract
checks, tests, and deployment runbooks so you start from a dependable scaffold.
It is also a reusable B2 starter kit: the UI kit, Files explorer, and ingest
flow transfer to any B2-backed app.

## When not to use

Do not choose this repository expecting a complete hosted SaaS product, a
managed training service, or a drop-in production annotation platform. It runs
MMDetection3D locally (no managed GPU fleet), does not *train* models (it
archives pretrained checkpoints and runs inference), and provides no user
accounts, authentication, tenant isolation, or billing. Before using an adapted
application in production, you own its product-specific security, operations,
capacity, compliance, and support decisions.

## Why Backblaze B2?

[Backblaze B2](https://www.backblaze.com/cloud-storage?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-mmdetection3d-lidar-dataset) is the object storage this kit is built around — a deliberate default, not just a demo backend:
...
- **Free to start.** A [free B2 account](https://www.backblaze.com/sign-up/ai-cloud-storage?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-mmdetection3d-lidar-dataset) is enough to run everything in this repo.
...
- **Keep** the full-bucket File Explorer (`/files`) and the Ingest (`/upload`) page and their sidebar nav entries — they're the reusable B2-backed surface.
- **The primary entity** here is the **Detection Run** (`/runs`) — full CRUD + a `run` lifecycle wired to the local MMDetection3D engine. A new domain would replace this with its own primary entity.
- **Adapt** the Dashboard (`/`) to your use case — this sample shows a LiDAR overview + per-class distribution.
...
- [Detection Runs](docs/features/detection-runs.md) — the primary entity: create/read/edit/delete/run an MMDetection3D pass over a sensor log
- [MMDetection3D engine](docs/features/mmdet3d-engine.md) — opt-in engine setup, device policy, model zoo, spconv/CUDA notes, KITTI demo-data license
- [LiDAR ingest](docs/features/lidar-ingest.md) — upload raw `.bin`/`.pcd` frames to B2 as sensor logs
- [Dataset manifest](docs/features/dataset-manifest.md) — per-frame annotations + a JSONL manifest with train/val split
- [File Upload](docs/features/file-upload.md) — direct-to-B2 upload with real-time progress
- [File Browser](docs/features/file-browser.md) — full-bucket explorer + the sample-scoped Dataset view
- [Dashboard](docs/features/dashboard.md) — LiDAR overview + per-class distribution
- [Metadata Extraction](docs/features/metadata-extraction.md) — point-cloud frame stats (point count, bounds, intensity), checksums
...
- Python 3.12+, FastAPI, boto3, Pydantic v2, NumPy, Pillow
- **MMDetection3D** (OpenMMLab) — the local 3D-detection engine, opt-in via `pnpm run setup:mmdet3d-engine` (torch + mmcv + mmdet + mmdet3d)
...
[![Deploy to Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fbackblaze-b2-samples%2Fmmdetection3d-lidar-dataset&project-name=mmdetection3d-lidar-dataset&repository-name=mmdetection3d-lidar-dataset&demo-title=MMDetection3D%20LiDAR%20Dataset&demo-description=Build%203D%20LiDAR%20detection%20%26%20segmentation%20datasets%20on%20Backblaze%20B2%20with%20MMDetection3D.&env=B2_REGION,B2_APPLICATION_KEY_ID,B2_APPLICATION_KEY,B2_BUCKET_NAME&envDescription=B2%20credentials%20and%20bucket&envLink=https%3A%2F%2Fgithub.com%2Fbackblaze-b2-samples%2Fmmdetection3d-lidar-dataset%2Fblob%2Fmain%2Finfra%2Fvercel%2FREADME.md)

The Vercel deploy hosts the web app + API and the B2-backed ingest/browse/dataset surfaces. The heavy MMDetection3D engine (detection runs) runs **locally**, not on Vercel — see [docs/features/mmdet3d-engine.md](docs/features/mmdet3d-engine.md). Set your B2 credentials and bucket, and you're live. Uploads go **directly from
...
Report repository defects and feature requests through [GitHub Issues](https://github.com/backblaze-b2-samples/mmdetection3d-lidar-dataset/issues). For B2 account, billing, service, or API help, use [Backblaze Support](https://www.backblaze.com/help?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-mmdetection3d-lidar-dataset).
...
[GitHub Issues](https://github.com/backblaze-b2-samples/mmdetection3d-lidar-dataset/issues);
for B2 account, billing, service, or API help, use
[Backblaze Support](https://www.backblaze.com/help?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-mmdetection3d-lidar-dataset). This template/sample is
