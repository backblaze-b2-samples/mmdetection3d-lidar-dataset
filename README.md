...
The **Dashboard** shows frames ingested, runs completed, 3D boxes, write amplification, and a per-class distribution chart. A **Detection Run** detail page shows each frame's bird's-eye-view preview and 3D-box annotations, with the dataset manifest and checkpoint downloadable from B2. Screenshots are generated by the publish pipeline.
